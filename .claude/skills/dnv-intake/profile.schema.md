...
`initial` — только первичная; `tie` — **не спрашивается при сборе контекста**,
добирается после одобрения, когда заполняется EX-17 (имена родителей,
провинция отделения, эпиграф tasa 012).

> Ветка `tie` существует, чтобы не удлинять первое интервью полями, которые
> понадобятся через месяцы. Все её поля `required: false`; до этапа TIE они
