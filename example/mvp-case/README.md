# MVP-кейс — синтетический сквозной пример (тонкий срез)

Полностью **вымышленный** кейс для проверки премисы движка: «незнакомец
безопасно получает корректный черновик». Один архетип v1 (autónomo/контрактор +
cónyuge, renovación). Реальных значений здесь нет (денилист по `example/` = 0).

Это фикстура **MVP-гейта** (U19): см. `docs/MVP_GATE.md`.
...
## Тонкий срез: intake → draft → field-QA
