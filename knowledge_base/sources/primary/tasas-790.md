...
> **Что это:** карта кодов tasa Modelo 790, релевантных пайплайну DNV, каждый со
...
> ⚠️ **Сумм здесь нет намеренно.** Правило `_private/RESEARCH_TODO.md` п. 3:
> цифры tasa не переносятся в базу даже из сходящихся между собой вторичных
> источников — они переписывают друг друга. Размеры установлены **Orden
...
> странице `sede.inclusion.gob.es` — здесь он не воспроизводится: PII-гейт репо
> требует, чтобы любой email в трекаемом файле был из зарезервированного диапазона
> RFC 2606, и институционального исключения из этого правила нет (см.
> `_private/PII_GATE_NOTES.md` §5).
...
**Где 052 в пайплайне DNV встречается легитимно:** только на **autorización de
regreso**, потому что её выдаёт не UGE-CE, а Delegación/Subdelegación del Gobierno
или полиция (art. 5.4 RD 1155/2024 — см. `rd-1155-2024-extractos.md`). См.
`../../norms/tie-huellas.md` и навык `/dnv-tie`.
...
- Разбор этапа — `../../norms/tie-huellas.md`, навык `/dnv-tie`.

---

## Сводка для чеклиста

| Этап пайплайна | Код | Кому |
|---|---|---|
...
| Autorización de regreso (выезд во время рассмотрения) | **790-052** | на того, кто выезжает |
