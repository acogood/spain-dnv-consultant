...
многосессионным пайплайном** (KTD11) и позволяет каждому навыку **проверять
предусловия** и останавливаться с инструкцией, а не догадываться (KTD12).
...
> **Вне графа:** `dnv-status` (read-only консюмер состояния, ничего не пишет) и
...
**Тонкий MVP-срез** (использован для U19-гейта): `dnv-intake → dnv-documents →
...
**На входе (precondition-гард, KTD12):**
...
Онбординг (`README`, U13) использует это же состояние.
...
записывай сюда NIE/имена/суммы (дата-минимизация; KTD13).
